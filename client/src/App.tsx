  ...
  return (
    <div className='p-3 max-w-[600px] my-0 mx-auto'>
      ...
      <form className='my-4 border border-black p-3' noValidate ref={formRef}>
        ...
        <button
          type='submit'
          className='rounded-full uppercase bg-blue-500 hover:bg-slate-600 transition-all duration-100 text-white px-3 py-1 text-[14px]'
          onClick={handleClick}
        >
          Enviar mensagem
        </button>
      ...
      <div
        ref={chatRef}
        className='chat fixed z-[99999] bottom-0 left-[50%] translate-x-[-50%] w-full max-w-[400px] h-[300px] overflow-y-scroll border-t border-r border-l border-black rounded-t-lg'
      >
        {state.receivedMessages.map((el, i) => (
          <div
            key={i}
            className={`flex gap-[15px] py-1 px-4 ${el.id === state.username && 'flex-row-reverse'}`}
          >
            <div className='flex flex-col items-center'>
              <span
                ...
                className={`rounded-full flex justify-center items-center text-lg font-bold p-2 w-[60px] h-[60px]`}
              >
                {el.id.charAt(0)}
              </span>
              <p>{el.id}</p>
